...
def process_course_update(course_id: int) -> None:
    ...
    subscribes = Subscribe.objects.filter(
        course=course_id, is_archived=False
    ).select_related("user")
    # Получаем наименование курса
    try:
        course_title = Course.objects.values_list("title", flat=True).get(
            id=course_id
        )  # Получаем наименование курса
    ...
    for subscribe in subscribes:
        ...
        send_course_subscription_email.delay(
            user_email, course_title
        )  # Вызываем задачу для рассылки
...
def process_course_updates_cron() -> None:
    ...
    courses_to_notify = Course.objects.filter(
        updated_at__lte=four_hours_ago, notification_sent=False
    )
    ...
    for course in courses_to_notify:
        ...
        subscribes = Subscribe.objects.filter(
            course_id=course.id, is_archived=False
        ).select_related("user")
        ...
        logger.info(
            f"Запущена рассылка для курса «{course.title}» ({subscribes.count()} писем)."
        )
