...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        self.stdout.write(
            self.style.SUCCESS(
                "=== Запуск локальной инфраструктуры (Docker + Django + Celery) ==="
            )
        )
        ...
        try:
            ...
            if container_name in check_container.stdout:
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"-> Запуск существующего контейнера {container_name}..."
                    )
                )
                subprocess.run(
                ...
                self.stdout.write(
                    self.style.SUCCESS(
                        f"-> Скачивание образа и создание контейнера {container_name}..."
                    )
                )
                subprocess.run(
                    [
        ...
        processes["celery"] = subprocess.Popen(
            ...
            stdout=sys.stdout,
            stderr=sys.stderr,
        )  # Перенаправляем вывод в родительскую консоль
        ...
        try:
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"-> Остановка Docker-контейнера {container_name}..."
                )
            )
